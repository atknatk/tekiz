...
// Services and Models
import { PlatformKunyeModel, PlatformKunyesDataSource, PlatformKunyesPageRequested, OnePlatformKunyeDeleted, ManyPlatformKunyesDeleted, PlatformKunyesStatusUpdated } from '../../../../../../core/platform';
// Components
...
export class PlatformKunyesListComponent implements OnInit, OnDestroy {
	...
	displayedColumns = ['select', 'id', 'name', 'owner', 'actions'];
	...
	ngOnInit() {
		...
		**/
		const paginatorSubscriptions = merge(this.sort.sortChange, this.paginator.page).pipe(
			tap(() => this.loadPlatformKunyesList())
		)
		.subscribe();
	...
	loadPlatformKunyesList() {
		...
		this.store.dispatch(new PlatformKunyesPageRequested({ page: queryParams }));
		this.selection.clear();
	}
	...
	deletePlatformKunyes() {
		...
		dialogRef.afterClosed().subscribe(res => {
			...
			this.store.dispatch(new ManyPlatformKunyesDeleted({ ids: idsForDeletion }));
			this.layoutUtilsService.showActionNotification(_deleteMessage, MessageType.Delete);
			this.selection.clear();
		});
	}
	...
	updateStatusForPlatformKunyes() {
		...
		dialogRef.afterClosed().subscribe(res => {
			...
			this.store.dispatch(new PlatformKunyesStatusUpdated({
				status: +res,
	...
	editPlatformKunye(platformKunye: PlatformKunyeModel) {
		...
		saveMessageTranslateParam += platformKunye.id > 0 ? 'UPDATE_MESSAGE' : 'ADD_MESSAGE';
		const _saveMessage = this.translate.instant(saveMessageTranslateParam);
		const _messageType = platformKunye.id > 0 ? MessageType.Update : MessageType.Create;
		const dialogRef = this.dialog.open(PlatformKunyeEditDialogComponent, { data: { platformKunye } });
		dialogRef.afterClosed().subscribe(res => {
			if (!res) {
				return;
			}
	 ...
	 */
	masterToggle() {
		if (this.selection.selected.length === this.platformKunyesResult.length) {
			this.selection.clear();
		} else {
			this.platformKunyesResult.forEach(row => this.selection.select(row));
		}
	}
